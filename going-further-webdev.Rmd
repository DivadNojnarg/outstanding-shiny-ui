...
```{r, eval=FALSE}
packer::npm_install(c("babel", "babel-loader", "@babel/core", "@babel/preset-env"), scope = "dev")
```
...
```{r, eval=FALSE}
packer::npm_install(c("framework7", "echarts", "echarts-gl"), scope = "prod")
```
...
```{r, eval=FALSE}
packer::npm_install(c("framework7-loader", "@babel/preset-react"), scope = "dev")
```
...
We have to create a `./.babel.rc` script that will add some configuration for Babel to handle JSX:
...
We can ultimately bundle our JS code and check the app works:
...
```{r vdpMod-framework7-resources, echo=FALSE, fig.cap='Framework7 assets in node modules', out.width='100%'}
...
```{r vdpMod-basic-app-2, echo=FALSE, fig.cap='vdpMod basic app with main app component template', out.width='100%'}
...
Inside `./srcjs/components/app.f7.jsx`, we import the newly designed component and replace `{content}` by `<VdpWidget label="Van der Pol Model"/>`:

```js
import VdpWidget from './vdp.f7.jsx';
```

On the R side, you may add this in the server code to track the gauge value:
...
```{r vdpMod-basic-app-3, echo=FALSE, fig.cap='vdpMod basic app with range slider', out.width='100%'}
...
To work properly, echarts requires to setup an HTML empty container in the DOM, that will receive the plot, specifying the height and width in pixels, and its __id__:

```html
<div id="line-plot" style="width:600px; height:400px;"></div>
```

On the JS side, we instantiate the plot and set the options list to give the plot a title, set the axis labels, add the time series:

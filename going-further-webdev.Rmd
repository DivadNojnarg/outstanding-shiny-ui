...
The UI will contain a slider input to change $\mu$ as well as two plots, one for the solutions `x` and `y` and another for the __phase plan__ analysis including the vector field and a simple trajectory from a chosen initial condition. In short, phase plan [analysis allows to __qualitatively__ study a non linear systems, that is determine:
  ...
  - What are the __steady states__ (points for which the system).
...
For convenience, we import CSS directly inside the JS code, thereby requiring style loaders:
...
Interestingly, Framework7 allows to write JSX code without the need of importing React. This is very convenient but require to add an extra [loader](https://www.npmjs.com/package/framework7-loader) as well as a babel react component:
...
We modify the `./srcjs/config/loader.json` to include the required loaders for `js`, `jsx` and `css` files.

...
as shown in Figure \@ref(fig:mobile-twitter).

```{r mobile-twitter, echo=FALSE, fig.cap='Twitter design on mobile devices.', out.width='30%', fig.align = 'center'}
knitr::include_graphics("images/mobile/mobile-twitter.png")
```

The goal of this short chapter is to introduce you to mobile development for Shiny. We are going to
...
Mobile app development consists of developing an application for mobile devices, such as
tablets and phones (Android, iPadOS). We call a __native__ app when it is dedicated to the specified
...
more consistent, they may be installed via __stores__ (App store, google play), and they can work __offline__. 
...
capabilities by being able to have a __full-screen__ display, being installable and providing a launch screen,
like native apps with a consistent UI. They must obey three rules:
...
There are several criteria to meet the installable state:
  ...
  - It must include a __manifest__, that is a __JSON__ file specifying app __metadata__ like its name, short name (`short_name`), icons to use
  ...
  - The app must have a registered __service worker__, which can __cache__ the app content, thereby making sure to provide offline
  support.
  
Once all criteria are fulfilled, the web browser may show an __installation prompt__ to start the process.
  
### How to develop a PWA?
Nowadays, tools exist to develop native-looking apps with a common language, JavaScript. 
This is the case with [Framework7](https://framework7.io/). `{shinyMobile}` (Figure \@ref(fig:mobile-shinyMobile)) was developed using this template. Other tools like the Google __PWA__ compatibility [script](https://github.com/GoogleChromeLabs/pwacompat) 
significantly reduce the pain to make our apps installable on multiple platforms. 

```{r mobile-shinyMobile, echo=FALSE, fig.cap='Twitter design on mobile devices.', out.width='50%', fig.align='center'}

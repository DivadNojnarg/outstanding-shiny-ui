...
```{r, echo=FALSE, results='asis'}
code <- '-- Failure (<text>:2:3): validate width works --
...
`tabler_card(width = "hello world")` did not throw an error.'
exclude_from_html(code)
```
...
The returned list contains one extra dependency, thereby breaking the underlying test. This subsequently requires to change the test. 

### Testing template behavior
Testing the JavaScript behavior is one of the biggest challenge to validate a template. If the R component has been carefully validated, it does not mean that its JavaScript effects are. For instance, let's consider the `tabler_progress()`  that may be updated with `update_tabler_progress()`. How do we check whether the progress value is correctly set?
...
to inspect the current app, as shown Figure \@ref(fig:tabler-crrry-debug). In practice, open a web browser window and type `localhost:<REMOTE-DEBUGGING-PORT>`. Follow the instructions and select the `Console` tab within the developer tools. 
...
After running the above code, browse to `localhost:<REMOTE-DEBUGGING-PORT>` and you should get what is shown on Figure \@ref(fig:tabler-crrry-debug-3).

...
validate_tabName <- function(tabName) {
  ...
  if (temp) {
    stop(
      paste(
        "Please do not use punctuation characters like",  
        wrong_selector, 
        "in tabNames. This might cause JavaScript issues."
      )
    )
  }
}
...
p <- processx::process$new(
  ...
  c(
    "-e",  
    "options('shiny.port'= 3515); OSUICode::update_tabler_navbar_example()" 
  )
)

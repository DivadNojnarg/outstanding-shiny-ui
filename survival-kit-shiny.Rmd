...
There is also a plug and play `toggle` method. However, this won't work out of the box?
...
$.extend(boxBinding, {
  ...
  subscribe: function(el, callback) {
    let collapsedBtn = $('#mybox').find('[data-widget="collapse"]');
    $(collapsedBtn).on('click', function(event) {
      setTimeout(function() {
        callback();
      }, 550);
    });
...
Some comments about the binding:
  ...
  - `setValue` search for a children of our box that holds the `[data-widget="collapse"]` attribute (in other words, this is the collapse button) and triggers a click on it
  - `subscribe` listens to the `click` event and delays the `callback` call by a

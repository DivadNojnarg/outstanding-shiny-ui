...
##### Initialize inputs
`initialize(el)` is called before the input is bound but not all input need to be initalized. Some API like [Framework7](https://framework7.io) require to have an initialize method. Below is an example for the toggle input:
...
##### Get the value
`getValue(el)` returns the input value. The way to obtain the value is different for almost all inputs.

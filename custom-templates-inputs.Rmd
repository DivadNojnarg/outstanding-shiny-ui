...
In the previous chapter, we built template __dependencies__, the page __skeleton__, as well as containers like cards. Now, it is a great time to integrate new inputs, leveraging all knowledge from Chapter \@ref(shiny-input-system).
...
In Tabler, the button __status__ is mandatory, which is the reason why it is a function parameter. Moreover, we add an horizontal right margin to the icon, if provided so that the label renders well (`mr-1`, where m stands for margin, r is the right direction and 1 is the margin value). We assume that by default, the button have a blue color, that is given by `btn-primary`:
...
Besides, we may also create an `update_tabler_switch()` function similar to the `updateCheckboxInput()`. We leverage `shiny:::dropNulls`, a function that removes all `NULL` elements from a list. If you remember, the `session$sendInputMessage` from R is received by the `receiveMessage` method on the JavaScript side:
...
As stated in Chapter \@ref(shiny-input-system), it is quite straightforward to bind other elements than primary inputs (HTML elements with the input tag) to Shiny. As a reminder, we created a custom input binding to detect the state of a `{shinydashboard}` box.
...
Where do we start? First of all, we add an __id__ attribute to the `tabler_navbar_menu()` so that it holds the corresponding `input$id`. Whether to use `inputId` or `id` as a parameter name is up to you, ìd` being personally prefered in that case:
...
3. Cards are a central elements of all templates. So are tabset panels! Try to modify the `tabler_card()` function

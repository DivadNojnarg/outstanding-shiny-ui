...
In Tabler, the button __status__ is mandatory, which is the reason why it is a function parameter. We assume that by default, the button have a blue color, that is given by `btn-primary`:
...
The button tag is identical to that of Shiny, reason why we don't show it here. The whole code may be found [here](https://github.com/DivadNojnarg/outstanding-shiny-ui-code/blob/b040a24e576f5d190825be0433edac288bbfbc26/R/tabler.R#L622).

```{r, echo=FALSE, results='asis'}
code_chunk(OSUICode::get_example("tabler/tabler-button"), "r")
```
...
We proceed to the main wrapper creation that has is a `label` tag having the `form-check form-switch` class as well as an optional `width` parameter:
...
```{r, echo=FALSE, results='asis'}
code_chunk(OSUICode::get_example("tabler/tabler-switch"), "r")
```
...
Hence, it makes more sense to listen to `shown.bs.tab`, that is wait the current tab to be shown:
...
```{r, echo=FALSE, results='asis'}
code_chunk(OSUICode::get_example("tabler/tabler-update-navbar", view_code = FALSE), "r")
```

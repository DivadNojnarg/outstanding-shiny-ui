...
__Note__: we don't assign `app` to a variable to make it globally accessible by any script!
...
To fix this issue, Framerwork7 has a `clicks` module options, that we may pass any valid CSS selector to the `externalLinks` property. Below is the default configuration:
...
__Note__: one might be tempted to target all `a` elements: this is a bad idea, 
...
Alternatively, you could also put all parameters in __options__ 
...
__Note__: As far as I have tested, some options won't work like the __global theme__, which 
...
### Order matters {#shinyMobile-order-matters}
...
```js
app.component.create(parameters);
```

`build_js` aggregates `component.js` and `init.js` in that order, which raises an error since `app` 
is used before being defined. Remember that `build_js` has a __files__ parameter to specify a custom order and 
...
build_js(files = paste0("srcjs/", c("init.js", "component.js", jsFiles)))
```


### Global data and methods {#mobile-global-data}
The Framework7 __data__ parameters allows to share global elements between multiple scripts. 
...
return an empty `tooltips` array. We update the `tooltips.js` file so that we:
  ...
  - Check if the tooltip instance is already in the `app.data` cached before creating an instance.
...
Similarly __methods__ hosts all global app methods. These are function you may re-use in different parts. Like for `app.data`, we access them with `app.methods`. 

__Important note__: `app.data` has been removed since Framework7 6.0.0!

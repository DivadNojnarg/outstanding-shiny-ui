...
It is a flexible toolkit to build __native-looking__ web apps or progressive
web apps (PWA). It has an incredibly rich ecosystem:
  ...
  - Hundreds of components with different looks for __iOS__ and __material design__, even desktop. 
...
We browse the [package](https://framework7.io/docs/package.html) page,
...
Once the package is created and opened, we download Framework7 dependencies, specifying
...
The `body` tag is composed of several layers. The first one is the `app` __root__ element, which is crucial to the template __initialization__. We'll come back to that point in the next section. 

```{r, echo=FALSE, results='asis'}
html_code <- '<!-- App root element -->
<div id="app">
...
Then, we find the __view__ component, inside which is located the __page__,  which hosts the __navbar__, __toolbar__ (bottom bar) and the __page content__.

```{r, echo=FALSE, results='asis'}
html_code <- '<!-- Top Navbar -->
<div class="navbar">
...
the toolbar may contain items in a `...` parameter. In practice, they may contain more elements, but this is enough for now:
...
owing to the `esbuild` JS bundler. Note that this is a more modern version of writing equivalent to the `module.exports` and `require` shown in section \@ref(about-js-modules).
...
```{r, echo=FALSE, results='asis'}
js_code <- "export const setConfig = () => {
...
2. Add to it a `setTouchStyle` function (copy and paste code from section \@ref(mobile-shinyMobile-other)):

```{r, echo=FALSE, results='asis'}
js_code <- "const setTouchStyle = (config) => {
  if (config.hasOwnProperty('touch')) {
    if (config.touch.tapHold) {
      // touch logic
    }
...
3. Add a `setColorTheme` function (copy and paste code from section \@ref(mobile-shinyMobile-theming)):

```{r, echo=FALSE, results='asis'}
js_code <- "const setColorTheme = (config, app) => {
...
4. Add a `setFilledTheme` function (copy and paste code from section \@ref(mobile-shinyMobile-theming)):

```{r, echo=FALSE, results='asis'}
js_code <- "const setFilledTheme = (config) => {
...
5. Add the `setDarkMode` function:

```{r, echo=FALSE, results='asis'}
js_code <- "const setDarkMode = (config, app) => {
  if (!config.hasOwnProperty('dark')) config.dark = false;
  if (config.dark) {
    app.methods.toggleDarkTheme();
  }
...
```{r, echo=FALSE, results='asis'}
js_code <- "export const initTheme = (config, app) => {
...
7. Move it to the `init.js` script and import the `helpers_theme.js`.

```{r, echo=FALSE, results='asis'}
js_code <- "import { initTheme } from './helpers_theme.js'"
code_chunk_custom(js_code, "js")
```

...
As a reminder, the code examples shown throughout this chapter are gathered in a package accessible [here](https://github.com/DivadNojnarg/installable-pwa-test). 
:::
...
the latest stable version, that is `5.7.14`, and targeting `bundle.min` files:
...
The `head` tag contains meta tags, some of them required for the PWA features, discussed in the next Chapter \@ref(mobile-pwa). 
...
Then we find the `view view-main` component, inside which is located the `page`, containing
the `navbar`, `toolbar` (bottom bar) and the `page content`. Altough, Framework7 inserts JavaScript code
at the end of the body, we can include them in the `head` as well. 

Leveraging `html_2_R`, we convert the above code to R and extract the `f7_page` function. We replace
the title content by a title parameter, paying attention to remove all CSS and JS links, as they are already included with the `add_dependencies` function applied to `body`. The page content is contained in a `...` parameter.
...
In the following parts, we are going to create multiple JS files. If we define app in `init.js`, how do we retrieve it in another script? If you remember about the JavaScript module part \@ref(about-js-modules), this is a good place to exploit them. Any element may be exported and made available to other scripts with:

```js
// Export in one script like init.js
export let myVar = 'test';
export function myFunc() {
  console.log('Plop');
}

// Import in another script like widgets.js
import { myVar, myFunc } from 'path_to_init'
```

::: {.warningbox data-latex=""}
`export` and `import` must be called at the top level of a script. For instance,
the `export` in 'init.js' cannot be inside the `$( document ).ready(function(...)});`, as this would
trigger an error with the terser. 
:::
...
// Export app to another script: uncomment 
// when considering multiple scripts!
// export app;
```

::: {.warningbox data-latex=""}
Mostb of the code inside `init.js` is wrapped in a function that waits the document to be ready before interacting with the DOM. This is necessary since the configuration is retrieved from a `script` DOM element inserted when the application starts, thus not immediately available. If you try to remove the `$( document ).ready(function(...)});`, an error will be raised since the configuration cannot be properly retrieved (timing issue). 
::: 
...
initialize other widgets, interact with layout elements and listen to app events (offline, online, ...). 
...
From now, we can add a welcome notification message with the [notification](https://framework7.io/docs/notification.html) API. There are 2 steps:
...
app parameters are accessible with the `.` notation. For instance, if you set a version number, it is accessible later
with `app.version`.
...
current device stored in `app.device`, and accordingly adapt the design. Nothing prevents you from forcing the
iOS layout on android devices, even though being quite irrelevant. Here are many properties to [review](https://framework7.io/docs/device.html).
...
test this global feature. Note the use of the internal Framework7 [utils](https://framework7.io/docs/utils.html#nexttick) `app.utils.nextTick`, which is nothing more than a `setTimeout`. At this point, `init.js` should look like this:
...
advanced actions. Do you remember the `app.views.create('.view-main');` step? This real purpose is actually to
...
As the `config` object may be re-used in multiple scripts, it is a good idea to include it
in the `init.js` export list:
...
and you only need to add one line of documentation in the roxygen part. Alternatively, you could also put all parameters in __options__ and provide a default value for `f7Page` like:
...
and write `var app = new Framework7(config);` on the JS side. 
...
To illustrate this feature, we consider the `tooltip` [example](https://v5.framework7.io/docs/tooltip.html). 
...
`updateF7Tooltip` sends three elements to JS: the tooltip target element `message.targetEl`, the optional new text, that is `message.text` and the action to perform `message.action` (either update or toggle state).
...
Similarly __methods__ hosts all global app methods. These are functions you may re-use in different parts. Like for `app.data`, we access them with `app.methods`. 
...
// exports to reuse app and config in multiple scripts
export {app, config};
```
...
We have now set all base pieces. In the next chapter, we see how to convert a `{shinyMobile}` app (and any vanilla shiny app) into a progressive web app.
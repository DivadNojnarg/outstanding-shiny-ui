...
For now, let's just erase the `{creatR}` boilerplate (`argon_action_button.jsx`) with the above code an recompile with `yarn run webpack`. We also have to tell Shiny we want to
...
which will also update the `package.json` file. 
...
The slider JS API is actually taken from the well known [noUiSlider](https://refreshless.com/nouislider/) library and is already available in the package node modules. Still in the same package as the action button input, we can call:
...
reactShinyInput('.argon_slider', 'mypkg.argon_slider', ArgonSlider);
```

At that stage, we modified the `webpack.config.js` file so that there are multiple entry 
...
The reason is quite simple: we did not set the `setValue` inside our JSX code yet.
...
Let's add the `onSet` prop to the `ArgonSlider` component, that is basically fired once
...
What value should we pass to `setValue`? There is no easy way to recover the slider value
directly within the React component. However, this is something quite easy to do inside the
input binding. We have to modify the `setValue` method, which does not recover the slider instance. To change the binding method, we wait for shiny to be connected, we unbind all inputs and search inside the `Shiny.inputBindings` registry (See \@ref(edit-input-binding)) providing the binding name,that is `mypkg.argon_slider`. The new value is obtained following the nouiSlider API:

```js
let sliderInstance = document
  .getElementById(el.id)
  .firstElementChild
  .noUiSlider;
let newValue = parseInt(sliderInstance.get());
```

We inject this value inside the existing `this.setInputValue(el, newValue)` method. The `parseInt` 
function ensures to get a number instead of a string. Once done, we rebind all inputs. The code below has to be added to the `argon_slider.jsx` code. 
...
$(document).on('shiny:connected', function(event) {
  ...
  $.extend(Shiny
    .inputBindings
    .bindingNames['mypkg.argon_slider']
    .binding, {
      setValue: function(el, value, rateLimited = false) {
        if ($(el).data('callback') !== undefined) {
          let sliderInstance = document
            ...
            .noUiSlider;
          let newValue = parseInt(sliderInstance.get());
          this.setInputValue(el, newValue);
...
Right now, the slider cannot be moved with the keyboard. Adding the `accessibility` prop
make it possible. This may correspond to a parameter provided in the `argon_slider_input` configuration: 
...
There are many more parameters that can be added to the following API. We leave it as an
exercise for the reader.

#### Exercise 2: a range slider

...
Note that `{reactR}` provides __webpack__ as JS manager. To __build__ the JS code we go to the RStudio terminal tab (or any terminal) and run at the package root:
...
`reactShinyInput` provides a wrapper to automatically __bind__ the input element. This is extremely convenient as it allows to solely focus on the component logic rather than binding it to the shiny system. It takes three parameters:
...
The expected `reactstrap` button HTML structure is rather simple including a title, the list of button tags and the selected item in a paragraph:
...
```{r react-button-group, echo=FALSE, fig.cap='Fully working React powered button group (similar to radio)', out.width='100%'}

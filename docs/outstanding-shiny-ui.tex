...
In the \href{https://github.com/RinteRface/shinyMobile/tree/master/inst/framework7-5.5.0}{shinyMobile} package, we chose a more robust approach. All bindings are contained in a folder and compressed so that we generate only one minified file containing all collapsed bindings. Why wouldn't we add all input bindings scripts to the template dependencies and load everything once for all? This would imply to add all dependencies even when the corresponding element don't exist, which is clearly not optimal.

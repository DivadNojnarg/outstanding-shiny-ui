...
Generally, R App developers lack a significant background in web development and often find this requirement overwhelming. It was this sentiment that motivated writing this book, namely to provide readers the necessary knowledge to extend Shiny's layout, input widgets and output elements. This project officially started at the end of 2018 but was stopped when Joe Cheng revealed the upcoming \href{https://mastering-shiny.org}{Mastering Shiny Book}. Fortunately, the later, does not cover the customization of Shiny user interfaces. Besides, this book may constitute a good complement to the work in progress \href{https://engineering-shiny.org}{Engineering Production-Grade Shiny Apps} by the ThinkR team, where the link between Shiny and CSS/JavaScript is covered.
...
Understanding how R and JS may communicate will help in the next section about the Shiny input system.
...
\begin{Highlighting}[]
\NormalTok{ui <-}\StringTok{ }\KeywordTok{fluidPage}\NormalTok{(}
  \KeywordTok{textInput}\NormalTok{(}\StringTok{"text"}\NormalTok{, }\StringTok{"My Text 1"}\NormalTok{),}
  \KeywordTok{hr}\NormalTok{(),}
  \KeywordTok{uiOutput}\NormalTok{(}\StringTok{"mytext"}\NormalTok{)}
\NormalTok{)}

\NormalTok{server <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(input, output, session) \{}
\NormalTok{  output}\OperatorTok{$}\NormalTok{mytext <-}\StringTok{ }\KeywordTok{renderText}\NormalTok{(input}\OperatorTok{$}\NormalTok{text)}
\NormalTok{\}}
...
Besides, we create the \texttt{updateBox} function, which will collapse the box:
...
The type parameter is obviously critical to make the connection between R and JavaScript and 2 different message handlers must have different types to avoid conflicts.
...
Below we quickly test if a tabler element renders well to see whether our dependency system is adequately setup. To that end, we include a random tabler element taken from the demo html page and include it as raw html, using \texttt{HTML}. We also ensure that basic Shiny input/output system works as expected with a sliderInput linked to a plot output.
...
Ok\ldots{} The layout is ugly, margins are not correct, the plot background does not match with the overall theme, \ldots{} but our info card and the shiny element work like a charm, which is a good start.

\hypertarget{the-body-content}{%
\subsection{The body content}\label{the-body-content}}

In this part, we translate the dashboard body HTML code to R. We create a function called \texttt{tabler\_body}. The \ldots{} parameter holds all the dashboard body elements and the footer is dedicated for the future \texttt{tabler\_footer} function.
...
\NormalTok{  navbar_cl <-}\StringTok{ "navbar navbar-expand-md"}
\NormalTok{  tags}\OperatorTok{$}\KeywordTok{header}\NormalTok{(}
    \DataTypeTok{class =}\NormalTok{ navbar_cl,}
\NormalTok{    tags}\OperatorTok{$}\KeywordTok{div}\NormalTok{(}
      \DataTypeTok{class =} \StringTok{"container-xl"}\NormalTok{,}
...
\NormalTok{      ),}
      ...
      \CommentTok{# brand stuff}
      \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\KeywordTok{is.null}\NormalTok{(brand_url) }\OperatorTok{||}\StringTok{ }\OperatorTok{!}\KeywordTok{is.null}\NormalTok{(brand_image)) \{}
...
\NormalTok{      \},}
      ...
      \CommentTok{# slot for dropdown element}
      \ControlFlowTok{if}\NormalTok{ (}\OperatorTok{!}\KeywordTok{is.null}\NormalTok{(nav_right)) \{}
...
\NormalTok{my_card <-}\StringTok{ }\KeywordTok{tabler_card}\NormalTok{(}\DataTypeTok{title =} \StringTok{"Ribbon"}\NormalTok{)}
...
\StringTok{  }\KeywordTok{tagAppendChild}\NormalTok{(}
    \KeywordTok{tabler_ribbon}\NormalTok{(}
      ...
      \DataTypeTok{bookmark =} \OtherTok{TRUE}
\NormalTok{    )}
\NormalTok{  )}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{ui <-}\StringTok{ }\KeywordTok{tabler_page}\NormalTok{(}
  \KeywordTok{tabler_button}\NormalTok{(}\StringTok{"btn"}\NormalTok{, }\StringTok{"Click"}\NormalTok{, }\DataTypeTok{icon =} \KeywordTok{icon}\NormalTok{(}\StringTok{"thumbs-up"}\NormalTok{), }\DataTypeTok{width =} \StringTok{"25%"}\NormalTok{)}
\NormalTok{)}

\NormalTok{server <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(input, output, session) \{}
  \KeywordTok{observe}\NormalTok{(}\KeywordTok{print}\NormalTok{(input}\OperatorTok{$}\NormalTok{btn))}
\NormalTok{\}}
...
\NormalTok{  value <-}\StringTok{ }\KeywordTok{restoreInput}\NormalTok{(}\DataTypeTok{id =}\NormalTok{ inputId, }\DataTypeTok{default =}\NormalTok{ value)}
\NormalTok{  inputTag <-}\StringTok{ }\NormalTok{tags}\OperatorTok{$}\KeywordTok{input}\NormalTok{(}
    \DataTypeTok{id =}\NormalTok{ inputId, }
...
\NormalTok{    inputTag}\OperatorTok{$}\NormalTok{attribs}\OperatorTok{$}\NormalTok{checked <-}\StringTok{ "checked"}
\NormalTok{  \}}
...
\NormalTok{ui <-}\StringTok{ }\KeywordTok{tabler_page}\NormalTok{(}
  ...
  \KeywordTok{tabler_button}\NormalTok{(}\StringTok{"update"}\NormalTok{, }\StringTok{"Go!"}\NormalTok{)}
\NormalTok{)}
...
In the \texttt{initialize} method, we ensure that if no tab is selected at start, the first tab will be by default. Otherwise, we select the activated tab. We use the string interpolation to ease the insertion of JS code in strings (\texttt{\$\{menuId\}\ .nav-link.active}).
...
value from the server side. Since it is not an input, implementing an input binding is inaprioprioate, so we decide to proceed with the \texttt{sendCustomMessage}/\texttt{addCustomMessageHandler} pair. We first create the \texttt{tabler\_progress}
tag which is mainly composed of:

\begin{itemize}
\tightlist
\item
  \texttt{style} gives the current value of the progress. This is the main element
\item
  \texttt{min} and \texttt{max} are bounds, in general between 0 and 100
\item
  \texttt{id} ensure that our progress bar is unique so that we may update it without
  conflict
\end{itemize}
...
On the JS side, we have a basic \texttt{addCustomMessageHandler}. As mentionned in Chapter \ref{survival-kit-javascript}, \texttt{sendCustomMessage} and \texttt{addCustomMessageHandler} are connected by the same \texttt{type}. This is crucial! Moreover, as the sent message is a R list,
it becomes an JSON, meaning that elements must be accessed with a \texttt{.}:
...
NOTE: How to handle custom messages in shiny modules? Well, it is pretty straightforward: we wrap any id with the module namespace given by \texttt{session\$ns()} before sending it to JS. You may even do it by default (without modules) like I did in the previous example since \texttt{session\$ns()} will be \texttt{""}.

\hypertarget{tab-events}{%
\section{Tab events}\label{tab-events}}

Do you remember about the navbar element and the tabsetpanel (also called navs) system of Chapter \ref{custom-templates-skeleton}? Navs allow to organize any app into several tabs, acting like pages. This is a powerful tool for Shiny since it is currently not straightforward to create \href{https://community.rstudio.com/t/shiny-app-composed-of-many-many-pages/7698}{multi-pages} Shiny apps like anyone would do with a website. Navs relies on the Bootstrap4 API but we only used few JS functions. There exist \href{https://getbootstrap.com/docs/4.0/components/navs/\#events}{events} that trigger after/before showing/hiding a tabs, that is \texttt{hidden.bs.tab} and \texttt{shown.bs.tab} (\texttt{hide.bs.tab} and \texttt{show.bs.tab} are triggers before). For instance, we may recover
...
Toasts are mainly composed of a header and a body, wrapped together. There might be a close button in case the toast does not hide itself. If multiple toasts appear one after each others, they are stacked, the latest being at the bottom of the stack. The position is controled with the style attribute like \texttt{style="position:\ absolute;\ top:\ 0;\ right:\ 0;"} for a top-right placement. Accessibility parameters like \texttt{aria-live} are detailed \href{https://getbootstrap.com/docs/4.3/components/toasts/\#accessibility}{here}.
...
\item
  autohide hide automatically the toast (TRUE by default)
\item
  delay is the delay to hide the toast (500 ms)
\end{itemize}

There are 3 methods: hide, show and dispose (it ensures the toast does not appear anymore). Finally, we may fine tune the toast behavior with 4 events: show.bs.toast, shown.bs.toast, hide.bs.toast, hide.bs.toast, hidden.bs.toast (like for tabs).

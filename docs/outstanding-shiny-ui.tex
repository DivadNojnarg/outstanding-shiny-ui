...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Add an html dependency without overwriting existing ones}
\NormalTok{appendDependencies <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x, value) \{}
...
\NormalTok{  old <-}\StringTok{ }\KeywordTok{attr}\NormalTok{(x, }\StringTok{"html_dependencies"}\NormalTok{, }\OtherTok{TRUE}\NormalTok{)}
  
  \KeywordTok{htmlDependencies}\NormalTok{(x) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(old, value)}
\NormalTok{  x}
\NormalTok{\}}

\NormalTok{addDeps <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x) \{}
  \CommentTok{# below, the order is of critical importance!}
\NormalTok{  dashboardDeps <-}\StringTok{ }\KeywordTok{list}\NormalTok{(bs4_deps, jQuery_deps, tablers_deps)}
  \KeywordTok{appendDependencies}\NormalTok{(x, dashboardDeps)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

The \texttt{addDeps} function will be applied to the core HTML template, that remain to be designed!
...
- the content containing the dashboard body (does not mean \texttt{\textless{}body\textgreater{}} tag!!) as well as the footer
...
\CommentTok{# Below we quickly test if a tabler element renders well to see whether}
\CommentTok{# deps are correctly proccessed. We also ensure that basic Shiny input/output system works as expected}

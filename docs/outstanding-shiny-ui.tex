...
Do you remember about the navbar element and the tabsetpanel system of Chapter \ref{custom-templates-skeleton}? Navs allow to organize any app into several tabs, acting like pages. This is a powerful tool for Shiny since it is currently not straightforward to create \href{https://community.rstudio.com/t/shiny-app-composed-of-many-many-pages/7698}{multi-pages} Shiny apps like anyone would do with a website. Navs relie on the Bootstrap4 API but we only used few JS functions. There exists \href{https://getbootstrap.com/docs/4.0/components/navs/\#events}{events} that trigger after/before showing/hiding a tabs, that is \texttt{hidden.bs.tab} and \texttt{shown.bs.tab} (\texttt{hide.bs.tab} and \texttt{show.bs.tab} are triggered before). For instance, we may recover

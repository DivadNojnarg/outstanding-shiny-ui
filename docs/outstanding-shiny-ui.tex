...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{let}\NormalTok{ letters }\OperatorTok{=}\NormalTok{ [}\StringTok{"a"}\OperatorTok{,} \StringTok{"b"}\OperatorTok{,} \StringTok{"c"}\OperatorTok{,} \StringTok{"d"}\NormalTok{]}\OperatorTok{;}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# utility to add an html dependency without overwriting existing ones}
\NormalTok{appendDependencies <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(tag, value) \{}
  \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{inherits}\NormalTok{(value, }\StringTok{"html_dependency"}\NormalTok{))}
\NormalTok{    value <-}\StringTok{ }\KeywordTok{list}\NormalTok{(value)}
  
\NormalTok{  old <-}\StringTok{ }\KeywordTok{attr}\NormalTok{(tag, }\StringTok{"html_dependencies"}\NormalTok{, }\OtherTok{TRUE}\NormalTok{)}
  
  \KeywordTok{htmlDependencies}\NormalTok{(tag) <-}\StringTok{ }\KeywordTok{c}\NormalTok{(old, value)}
\NormalTok{  tag}
\NormalTok{\}}

\CommentTok{# add all dependencies to a tag}
\NormalTok{addDeps <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(tag) \{}
  \CommentTok{# below, the order is of critical importance!}
\NormalTok{  deps <-}\StringTok{ }\KeywordTok{list}\NormalTok{(bs4_deps, jQuery_deps, tablers_deps)}
  \KeywordTok{appendDependencies}\NormalTok{(tag, deps)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
...
##  $ meta      : NULL
##  $ script    : chr "jquery-3.5.0.slim.min.js"
##  $ stylesheet: NULL
##  $ head      : NULL
##  $ attachment: NULL
##  $ package   : NULL
...
Each of these element will be considered as an input parameter to the \texttt{tabler\_header} function, except the first element which is a default element and should not be removed. Morever, we will only show the brand element when it is provided. The \ldots{} parameter is a slot for extra elements (between the menu and dropdowns).
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tabler_header <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(..., }\DataTypeTok{brand_url =} \OtherTok{NULL}\NormalTok{, }\DataTypeTok{brand_image =} \OtherTok{NULL}\NormalTok{, nav_menu, }\DataTypeTok{nav_right =} \OtherTok{NULL}\NormalTok{) \{}
...
\NormalTok{      ),}
      ...
      \ControlFlowTok{if}\NormalTok{ (}\KeywordTok{any}\NormalTok{(brand_url, brand_image)) \{}
...
\NormalTok{          nav_menu}
\NormalTok{        ),}
        \KeywordTok{div}\NormalTok{(}\DataTypeTok{class =} \StringTok{"ml-md-auto pl-md-4 py-2 py-md-0 mr-md-4 order-first order-md-last flex-grow-1 flex-md-grow-0"}\NormalTok{, ...)}
\NormalTok{      )}
\NormalTok{    )}
\NormalTok{  )}
\NormalTok{\}}
...
Let's create the navbar menu. The \ldots{} parameter is a slot for the menu items. There is nothing else to say about this simple container.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tabler_navbar_menu <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(...) \{}
\NormalTok{  tags}\OperatorTok{$}\KeywordTok{ul}\NormalTok{(}\DataTypeTok{class =} \StringTok{"navbar-nav"}\NormalTok{, ...)}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

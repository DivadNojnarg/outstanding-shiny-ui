...
\texttt{initialize(el)} is called before the input is bound but not all input need to be initalized. Some API like \href{https://framework7.io}{Framework7} require to have an initialize method. Below is an example for the toggle input:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{// what is expected}
\KeywordTok{var}\NormalTok{ toggle }\OperatorTok{=} \VariableTok{app}\NormalTok{.}\VariableTok{toggle}\NormalTok{.}\AttributeTok{create}\NormalTok{(}\OperatorTok{\{}
...
\texttt{getValue(el)} returns the input value. The way to obtain the value is different for almost all inputs.

\hypertarget{set-the-value}{%
\paragraph{Set the value}\label{set-the-value}}

\texttt{setValue(el,\ value)} is used to set the value of the current input.

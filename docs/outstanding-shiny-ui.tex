...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{let}\NormalTok{ table }\OperatorTok{=}\NormalTok{ [...}\AttributeTok{Array}\NormalTok{(}\DecValTok{100}\NormalTok{).}\AttributeTok{keys}\NormalTok{()]}\OperatorTok{;} \CommentTok{// create an empty array of length 100 (so from 0 to 99, not from 1 to 100 like in R!!!)}

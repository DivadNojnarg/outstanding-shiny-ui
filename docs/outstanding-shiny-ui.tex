...
There is also a plug and play \texttt{toggle} method. However, this won't work out of the box?
...
\VariableTok{$}\NormalTok{.}\AttributeTok{extend}\NormalTok{(boxBinding}\OperatorTok{,} \OperatorTok{\{}
  ...
  \DataTypeTok{setValue}\OperatorTok{:} \KeywordTok{function}\NormalTok{(el}\OperatorTok{,}\NormalTok{ value) }\OperatorTok{\{}
    \AttributeTok{$}\NormalTok{(}\StringTok{'#mybox'}\NormalTok{)}
\NormalTok{      .}\AttributeTok{find}\NormalTok{(}\StringTok{'[data-widget="collapse"]'}\NormalTok{)}
\NormalTok{      .}\AttributeTok{click}\NormalTok{()}\OperatorTok{;}
  \OperatorTok{\},} 
  ...
  \DataTypeTok{subscribe}\OperatorTok{:} \KeywordTok{function}\NormalTok{(el}\OperatorTok{,}\NormalTok{ callback) }\OperatorTok{\{}
    \KeywordTok{let}\NormalTok{ collapsedBtn }\OperatorTok{=} \AttributeTok{$}\NormalTok{(}\StringTok{'#mybox'}\NormalTok{).}\AttributeTok{find}\NormalTok{(}\StringTok{'[data-widget="collapse"]'}\NormalTok{)}\OperatorTok{;}
    \AttributeTok{$}\NormalTok{(collapsedBtn).}\AttributeTok{on}\NormalTok{(}\StringTok{'click'}\OperatorTok{,} \KeywordTok{function}\NormalTok{(event) }\OperatorTok{\{}
...
\item
  \texttt{setValue} search for a children of our box that holds the \texttt{{[}data-widget="collapse"{]}} attribute (in other words, this is the collapse button) and triggers a click on it
\item
  \texttt{subscribe} listens to the \texttt{click} event and delays the \texttt{callback} call by a
  value which is slightly higher than the default AdminLTE2 animation to collapse the box (500mx). If you omit this part, the input will not have time to properly update!!!
\item
...
\StringTok{            setValue: function(el, value) \{}
\StringTok{              $('#mybox')}
\StringTok{                .find('[data-widget=}\CharTok{\textbackslash{}"}\StringTok{collapse}\CharTok{\textbackslash{}"}\StringTok{]')}
\StringTok{                .click();}
\StringTok{            \}, }
\StringTok{            receiveMessage: function(el, data) \{}
\StringTok{              this.setValue(el, data);}
\StringTok{            \},}
\StringTok{            subscribe: function(el, callback) \{}
\StringTok{              let collapsedBtn = $('#mybox').find('[data-widget=}\CharTok{\textbackslash{}"}\StringTok{collapse}\CharTok{\textbackslash{}"}\StringTok{]');}
\StringTok{              $(collapsedBtn).on('click', function(event) \{}

...
As we'll send a JSON (from the client), we leverage `fromJSON()` to properly treat the message. It is printed for debugging purposes and the value is injected inside a `hist(rnorm())` function.
...
The second task is to send a message to JS in order to update the gauge value. See it like an `updateSlider()` function for instance. We utilize `toJSON()` to send a random value to JS as well as a polite message:
...
It returns a list composed of:
  ...
  - Some __headers__ here indicating the content nature.
  - The __body__ which is what will be display upon client request. 

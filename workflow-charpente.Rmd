...
Moreover, `create_dependency` is able to filter all files, through the `charpent_options`:
  ...
  and is common in arabic countries for instance. 
  
You may imagine that `charpent_options` targets `bundle.min` files by default. If you don't find
any script, you probably have to set `bundle = FALSE` since some template don't have that 
flavor. 
...
which works like a charm. If you chose the local option, you also get an `inst` folder
with all relevant bulma files sorted by type. The `bulma-dependencies.R` script contains the 
...
The problem is that we don't necessarily know all package versions and may need
...
that you don't want bundles, minified, lite or rtl versions of scripts. 

...
\subsection{Set child/children}\label{set-childchildren}}
...
\OperatorTok{<!--}\NormalTok{chapter}\OperatorTok{:}\NormalTok{end}\OperatorTok{:}\DecValTok{03}\OperatorTok{-}\NormalTok{htmltools}\OperatorTok{-}\NormalTok{overview.Rmd}\OperatorTok{-}\NormalTok{->}

\CommentTok{# Dependency utilities \{#htmltools-dependencies\}}
\NormalTok{When creating a new template, you sometimes need to import custom HTML dependencies}
\NormalTok{that do not come along with shiny. No problem, htmltools is here }\ControlFlowTok{for} \KeywordTok{you}\NormalTok{ (shiny also }
\NormalTok{contains these functions).}

\CommentTok{## The dirty approach}
\NormalTok{Let}\StringTok{'s consider the following example. I want to include a bootstrap 4 card in a shiny app.}
\StringTok{This example is taken from an interesting question [here](https://community.rstudio.com/t/create-a-div-using-htmltools-withtags/22439/2).}
\StringTok{The naive approach would be to include the HTML code directly in the app code}
\end{Highlighting}
\end{Shaded}

...
We see in the head section (delimited by `<head></head>`) that Shiny has three dependencies:

  - jQuery
  - shiny (custom JavaScript and CSS)
  - Bootstrap (JavaScript and CSS) + other files (html5shiv, respond)
...
The last dependency contains custom JS and CSS code necessary for Shiny to work, that is:
  ...
  - Register input/output and handling every single input/output action. Detailed mechanisms are 
...
To sump up, all these libraries are necessary to make Shiny what it is! Importantly, Shiny only preloads what is
...
one slider in the app, which improves app performances. 
  
In Chapter \@ref(htmltools-dependencies), we explore tools to manage HTML dependencies. In Chapters \@ref(insert-tabs) and \@ref(shiny-custom-handler), we study a special case to insert dependencies during the app runtime.  

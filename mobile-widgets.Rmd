...
```{r, eval=FALSE}
f7_notif <- function(id, text, options = NULL, session = shiny::getDefaultReactiveDomain()) {

  if (!is.null(options$icon)) options$icon <- as.character(options$icon)

  message <- c(list(id = id, text = text), options)
...
we already created a cache for tooltips, so let's do it for gauges and notifications:
...
// server widgets
app.data[widget][message.id] = w;
```

Only the `id` location differs depending on the widget definition (ui vs server).
...
For each configuration element, we must check whether its class contains `shiny.tag` or `shiny.tag.list` and convert it to a character. Moreover, it may contain a nested list like:
...
The next step is to close the old instance if it belongs to a server widget (it may happen
that the widget is configured not to be automatically close. This avoids widgets to overlap), 
destroy the old instance, initialize the new instance and refresh 
the `app.data` cache:

```javascript
// Also close old instance
if (serverWidgets.indexOf(instanceFamily) > -1) {
  oldInstance.close();
}
...
We may also want to open the new instance if it's generated from the server like toasts or
notifications:

```javascript
// Open new instance if required
if (serverWidgets.indexOf(instanceFamily) > -1) {
  newInstance.open();
}
```

...
I must admit, there are few deviations like the navbar (`app.navbar.show()`) or the dialog but we have enough common points to design a main wrapper that creates any widget and update/open/close it.
...
The code examples shown throughout this chapter are gathered in a package accessible [here](https://github.com/DivadNojnarg/installable-pwa-test), specifically [here](https://github.com/DivadNojnarg/installable-pwa-test/blob/main/R/widgets.R) for widgets. 
:::
...
Pay attention to the `options$icon` element. As we can't convert shiny tags to JSON, it
...
opened. This is the reason why we added an `on` property to the message. All widgets trigger [events](https://v5.framework7.io/docs/notification.html#dom-events), 
...
nested for loop. The outer loop scans all `app.data` properties (ie widget categories), while the inner loop

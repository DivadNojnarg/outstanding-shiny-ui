...
```{r, eval=FALSE}
shinyAppDir(system.file("custom-handlers/say_hello", package = "OSUICode"))
```
...
$(function() {
  Shiny.addCustomMessageHandler('add-message-item', function(message) {
    ...
    let $items = $('.dropdown-menu').find('.dropdown-item').length;
...
```js
Shiny.addCustomMessageHandler("user-messages", function(message) {
  let id = message.id, action = message.action, content = message.body, index = message.index;
...
// other condition before ...
  ...
  let newMessage = '<div class="direct-chat-info clearfix">' +

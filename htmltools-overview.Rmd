...
|:----------:|:-------------:|
| children | Get all the direct descendants | 
| find     | Get all descendants            | 
| parent   | Get the direct ancestors       | 
...
There are currently two main methods to alter tag attributes, namely `$addAttrs()` (equivalent of `tagAppendAttributes`) and `$removeAttrs()`, even though more specific methods exists, for instance `$addClass()`, `$removeClass()` and `$toggleClass()`.
...
Below are listed some methods to alter the current tag or its children.

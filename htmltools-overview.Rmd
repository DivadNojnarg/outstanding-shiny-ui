...
Are you ready to become a tag witcher? ^[I hope you'll realize that it is easier than killing weird monsters, drinking dangerous potions made of deadly mutagens and not taking any bath for weeks...but one never knows!]. 
...
The result is shown in Figure \@ref(fig:htmltools-tabs-icons).
...
One of the strengths of the `tagQuery()` API is the ability to __chain__ methods, where the classic `{htmltools}` syntax might be __repetitive__ and heavy. `$resetSelected()` allows us to __reset__ the tag selection to the root tag after a given operation, thereby making it possible to chain multiple queries with different purposes. The overall flow is more human readable than a step-by-step approach, similar to the `tidyverse` or `ggplot` grammar. Let's combine all previous examples:
...
### Specific cases
There are situations where the previous methods won't work. What if you want to modify the
tab content from `Tab i` to `This is tab i`. A common mistake would be to proceed as follows:
...
#### Simple hooks
In this section, we'll see what is probably one of the most advanced `{htmltools}` feature, recently introduced in version [0.5.2](https://rstudio.github.io/htmltools/news/index.html#new-features--improvements-0-5-2). How would you conditionally render a tag, for instance depending on different conditions like external options, a specific theme version, ...?
...
Then we create our custom tag with the `.renderHook` parameter available for the `{htmltools}` tag element, providing a special function, namely a __render hook__, that will be called upon for tag rendering:
...
In Chapter \@ref(bslib-renderHook) we'll discuss another use case, which is about theme-dependent rendering, that is, for instance, rendering a tag differently whether Shiny is in Bootstrap 3 or Bootstrap 4 mode. 
...
We first consider the main wrapper, namely `my_wrapper_tag()`. The function creates a single `div` wrapper containing other items generated with another function. In the render hook, we simulate the impact of
a `theme_version` option. We capture all passed items in a list and apply `tagQuery()` to edit a given
element targeted with a specific class, using `$each()` to loop over all items and add an `id` attribute to them. We store the query result in a new item variable and replace the old items by the newly generated ones in the main wrapper with `$empty()` and `$append()`. Note the commented rows, we will discuss it later.
...
The next step is to design the nested item function. The tag consists of a
...
Note the `new-child` class. This is the one targeted one level up in `my_wrapper_tag()`.
...
The code does not seem to work as expected since the child items do not get any new `id` attribute,
...
which is exactly what we want! A real-life case study is available later in the book in section \@ref(bslib-bootstrap-accordion).

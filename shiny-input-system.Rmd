...
Clicking on next again exit the debugger. Interestingly, you'll notice that a text appears below the input, meaning that the `input$caption` element exists and 
...
The Shiny input binding system is too convenient to be only used for primary input elements like `textInput`, `numericInput`. It is a super powerful tool to unleash apps's interactivity. 
...
Let's provide some optimization and get rid of the `renderUI`. We proceed in 2 steps. The first part consists in customizing the `box` function to gather as many parameter as possible in an option list. For instance, we choose to extract background, width and title. width and background are expected to be numeric and character, respectively, while title might be any HTML tag, justifying the use of `as.character`.
...
```{r, eval=FALSE}
shinyAppDir(system.file("vignettes-demos/box-api", package = "shinydashboardPlus"))
```

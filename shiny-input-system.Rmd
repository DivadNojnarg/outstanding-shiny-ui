...
Let's provide some optimization and get rid of the `renderUI()`. We proceed in two steps. The first part consists in customizing the previously designed `box()` function from \@ref(boxes-on-steroids) to gather as many parameters as possible in a list of options. For instance, we choose to extract `width` and `title`. 
...
The whole JS code may be found in appendix \@ref(boxes-on-steroids-more-appendix). Below is the reworked version of the previously updated box without `renderUI()`. When app starts, the box is displayed as shown on Figure \@ref(fig:update-shinydashboard-box-optimized), which is better for the end user experience.

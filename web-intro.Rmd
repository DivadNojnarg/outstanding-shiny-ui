...
```{r dj-app, echo=FALSE, fig.cap='Shiny app with the Pioneer CDJ 2000 NXS2 professional gear look.', out.width='100%'}
...
What is this magic? Under the hood, this Shiny [app](https://github.com/DivadNojnarg/OSUICode/tree/266ad20f450fdc6a8c939216287b6d67bc9e828d/inst/intro/dj-system) only consists in:
...
Inside an HTML document, tag elements obey to the __box__ model, that briefly defines the element internal margins (padding), margins (space between multiple elements), the width and height. Elements are displayed according to the __flow layout__ model (Figure \@ref(fig:normal-flow-layout)). We distinguish block and inline elements:
  ...
  - __Inline__ elements (`<span></span>`, `<a></a>`) are printed on the same line. They can not contain block tags, for instance `<span><div><p>Hello World</p></div></span>` is not valid, but may contain other nested inline tags like `<a><img/></a>` (creates a clickable image pointing to a specific location). 
  - _Inline-block_ elements allow one to insert block element in an inline.
...
Below is how we would do with a Shiny app: 
...
```{r html-dom-edit, echo=FALSE, fig.cap='Modifications of the p element in the Hello World example.', fig.align = 'center', out.width='75%'}
...
```{r html-dom-search, echo=FALSE, fig.cap='Search for element having the "awesome-text" class.', fig.align = 'center', out.width='75%'}

...
The output is HTML code. For a R developer, being able to generate HTML code from R allows to remain focused on the main task instead of the web development burdens.
...
Inside an HTML document, tag elements obey to the __box__ model, that briefly defines the element internal margins (padding), margins (space between multiple elements), the width and height. Elements are displayed according to the __flow layout__ model (Figure \@ref(fig:normal-flow-layout)). We distinguish block and inline elements:
  ...
  - __Inline__ elements (`<span></span>`, `<a></a>`) are printed on the same line. They can not contain block tags, for instance `<span><div><p>Hello World</p></div></span>`, but may contain other nested inline tags like `<a><img/></a>`. The previous element creates a clickable image pointing to a specific location. 

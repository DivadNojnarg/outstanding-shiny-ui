...
<!-- Are these structure, control, or formatting tags? -->
  ...
  - __Inline__ elements (for instance `<span></span>`, `<a></a>`) are printed on the same line. They can not contain block tags but may contain other nested inline tags like `<a><span></span></a>`.  <!-- What does that do? -->
  - _Inline-block_ elements allow to insert block element in an inline.

Consider the following example. This is clearly a bad use of HTML conventions since an inline tag can not host block elements. <!-- So why show it? -->
...
<!-- Why should the reader care about the HTML inspector? What will it let them do? -->

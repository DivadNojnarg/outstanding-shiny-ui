...
#### HTML Inspector 101
Below, we describe the Chrome HTML inspector, also know as [Chrome DevTools](https://developers.google.com/web/tools/chrome-devtools), in details.
In addition to explore the HTML [structure](https://developers.google.com/web/tools/chrome-devtools/dom), the inspector allows to: 
...
Interestingly, a lot of Shiny app issues are related to JavaScript problems (htmlWidget conflicts, typos) that could be easily tracked if you know about the HTML inspector. We'll see in section \@ref(shiny-websocket) that you may also track websockets activity! 

Another important set of feature is the ability to switch between different devices, especially mobile platforms and to do a global performance audit with [Google LightHouse](https://developers.google.com/web/tools/lighthouse). The [book](https://engineering-shiny.org/when-optimize.html#tools-for-profiling) from Colin Fay et al. details the most relevant elements for Shiny app development.

Below, we propose at set of quick exercises to review the most important HTML inspector capabilities that we will use during Shiny app customization. We'll consider the app already defined above:
...
```{r html-dom-inspect, echo=FALSE, fig.cap='Inspection of the p element in the Hello World example'}
...
Alternatively, as depicted in Figure \@ref(fig:html-dom-edit), we could have done a right click on the `p` tag to display more options like:
  ...
  - Extract the CSS selector or JavaScript path (code to select the element): `body > div > p` and `document.querySelector("body > div > p")`, respectively. These 2 features are extremely powerful as they save you time to find these by yourself. Try to copy and paste `document.querySelector("body > div > p")` in the JavaScript console. Notice it returns the selected element, as shown in Figure \@ref(fig:html-dom-js-path)! Amazing isn' it?
  
```{r html-dom-js-path, echo=FALSE, fig.cap='Extract the JavaScript path to select the p element'}
...
```{r html-dom-edit, echo=FALSE, fig.cap='Modifications of the p element in the Hello World example'}
...
Whenever you are looking for a specific tag in a more complex page, the `search tag` option is a game changer (command + F within the Elements tab on a Mac). See Figure \@ref(fig:html-dom-search).

```{r html-dom-search, echo=FALSE, fig.cap='Modifications of the p element in the Hello World example'}

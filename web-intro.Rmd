...
Notice the output format is an example of an HTML tag, with an opening tag `<h1>` and closing tag `</h1>`. For a R developer, being able to generate HTML code from R allows to remain focused on the main task instead of the web development burdens.
...
```{r dj-app, echo=FALSE, fig.cap='Unleash the dj', out.width='100%'}
knitr::include_graphics("images/intro/dj-app.png")
```

If you are still not convinced, let's run this function from the `{OSUICode}` side package:
...
Wait until the blue waveform appears on the player's screen. Then click on the green play button to stop and start the music (the space key is also supported). Besides, the waveform is interactive, thereby allowing to browse through the current track. The rotating part (central part of the jog wheel) indicates the read position and other useful information. 

What is this magic? Under the hood, this Shiny [app](https://github.com/DivadNojnarg/outstanding-shiny-ui-code/tree/master/inst/dj-system) only consists in:

- 111 lines of CSS.
- 29 lines of JavaScript code.
- 2 png images (dj gear + rotating wheel).
- 36 lines of R code, including the `{wavesurfer}` htmlWidget [package](https://github.com/Athospd/wavesurfer) to display the waveform. 
- Few custom HTML tags. 
- Inspired by this [article](https://codepen.io/ruise/pen/MVPgrQ).
...
<!-- Are these structure, control, or formatting tags? -->
  ...
  - __Inline__ elements (for instance `<span></span>`, `<a></a>`) are printed on the same line. They can not contain block tags, for instance `<span><div><p>Hello World</p></div></span>`, but may contain other nested inline tags like `<a><img/></a>`. The previous element creates a clickable image pointing to a specific location. 
  - _Inline-block_ elements allow to insert block element in an inline.

Importantly, `<div>` and `<span>` are generic tags and don't have any semantic meaning, contrary to `<header>` and `<footer>`, which allow to structure the HTML page. `<div>` and `span` are used whenever no semantic block and inline container may be applied, respectively.
...
The developer tools are a crucial way to work with websites, and particularly customize Shiny apps. As shown in Figure \@ref(fig:html-dom), here are example of actions you will be able to perform: inspect the HTML structure of the page, debug JavaScript code as demonstrated in \@ref(shiny-js-inspector), inspect served files (static assets like CSS, JS, images), run performances audit \@ref(mobile-pwa), inspect websocket activity (section \@ref(shiny-websocket)) and many more. 

In this section, we restrict the description to the first panel (Elements) of the __developer tools__ [^inspector]. This feature is available in all web browsers, however for demonstration purposes, we will only focus on the [Chrome DevTools](https://developers.google.com/web/tools/chrome-devtools). It may be opened:

  - After a right click and selecting inspect.
  - After clicking on ctrl + shift (Maj) + I for Windows, ⌥ + ⌘ + I on Mac.
...
#### HTML and JavaScript
JavaScript is a game changer to give life to your web apps. It is an object oriented programming language allowing to interact with the HTML elements.
...
If you are not already familiar with JS, Chapter \@ref(survival-kit-javascript) provides some basic knowledge to start customizing your shiny apps.

## Summary
As demonstrated above, developing a shiny app is basically building a website from R and is completely compatible with the web languages, that is HTML, CSS and JavaScript. The remainder of this book presents more robust tools and concepts to extend Shiny.

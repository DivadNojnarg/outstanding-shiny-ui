...
As a reminder, the code examples shown throughout this chapter are gathered in a package accessible [here](https://github.com/DivadNojnarg/installable-pwa-test), specifically PWA apps are available [here](https://github.com/DivadNojnarg/installable-pwa-test/tree/main/inst/examples). 
:::
...
images being hosted in the `/www/icons` sub-directory.

The `create_manifest` function writes a JSON file at the provided location:
...
In practice, since the package already relies on other dependencies like Framework7, we will leverage the `charpente::add_dependencies` function to add all dependencies at once. 
...
We give it an id so as to call it later and edit the `beforeinstallprompt` event listener to show
the toast:
...
which is, as a reminder, an Framework7 internal equivalent to `setTimeout`. We first close the 
...
[{shiny.pwa}](https://github.com/pedrocoutinhosilva/shiny.pwa), that creates a PWA compatible structure
at run time, within the app `/www` folder. 

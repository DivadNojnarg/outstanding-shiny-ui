...
__Note__: it is good practice to add the `default!` tag after the variable definition, if you want
to let others modify it: `"$my-color: purple !default;"`
...
AdminLTE3 relies on Sass and all files are stored [here](https://github.com/ColorlibHQ/AdminLTE/tree/master/build/scss). Particularly, 
all variables are located in the `_variables.scss` partial. Since we can decompose our
Sass code in multiple layers thanks to `{sass}`, we can seamlessly customize the theme color
...
Let's provide our own default with some custom [colors](https://colorswall.com/palette/66354/):
...
`system.file("assets/AdminLTE-3.0.4/AdminLTE.scss", package = "fresh")`:
...
css <- sass(
  sass_layer(
    ...
    rules = sass_file(
      input = system.file("assets/AdminLTE-3.1.0/AdminLTE.scss", package = "fresh")
    )
  )
)
...
namely `3.1.0`, that is not necessarily the one we want. Therefore, an alternative could 
...
of Bootstrap 4, `4.5.3`, we also have to recover those dependencies in a separate folder. 
We finally edit the original `adminlte.scss` to change the Bootstrap Sass path:
...
`$sidebar-dark-bg` [variable](https://github.com/ColorlibHQ/AdminLTE/blob/master/build/scss/_variables.scss#L112) to be `$lightblue` and `$sidebar-light-bg` to `$lime`:
...
Notice you can retrieve all those information: [initial variables](https://bulma.io/documentation/customize/variables/#initial-variables) and [derived variables](https://bulma.io/documentation/customize/variables/#variables).
...
Particularly, we target the main body color hosted by `$scheme-main`. Instead of pointing

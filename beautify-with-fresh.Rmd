...
As shown in the previous chapter, __Sass__ is a powerful tool to customize apps in minutes, in addition to
drastically improve __code quality__, thereby empowering long term maintenance. Some CSS knowledge are required and it may not be that easy to find the corresponding variables to a specific purpose. For instance, would you be able to guess
what Bootstrap 4 variables are required to customize the `{bs4Dash}` template? 
...
```{r}
color_yiq <- "
  @function color-yiq($color, $threshold: 150, $dark: $yiq-text-dark, $light: $yiq-text-light) {
    $r: red($color);
    $g: green($color);
    $b: blue($color);
...
server <- function(input, output, session) {
  ...
  css <- reactive({
    # Recover the hex code since bs4dash_yiq does not accept names
...
```{r color-contrast-yiq, echo=FALSE, fig.cap='bs4dash_yiq in action', out.width='100%'}

...
An interactive live theming tool may be run from Github with an example shown in Figure \@ref(fig:fresh-live-theming):

```{r, eval=FALSE}
shiny::runGitHub("dreamRs/fresh", subdir = "inst/examples/create")
```

```{r fresh-live-theming, echo=FALSE, fig.cap='Live theming app with {fresh}.', out.width='100%', fig.align='center'}
knitr::include_graphics("images/beautify/fresh-live-theming.png")
```

## `{fresh}`, the big picture
`{fresh}` is built on top of `{sass}` and what you see later in Figures \@ref(fig:bs4Dash-fresh) and  \@ref(fig:shinydashboardPlus-fresh) may be done in few minutes. To design a new theme, the main function is `create_theme()`:
...
__theme__ allows you to import a [Bootswatch](https://bootswatch.com/) CSS theme; you may already know this if you ever used `{shinythemes}`. __output_file__ controls the output. If provided, `create_theme()` creates a CSS file at the specified location. Otherwise, it returns a string value containing the compiles CSS. It is useful in a package context where you don't necessarily want to generate a new theme each time. __...__ is where we pass a list of CSS variables through `bs4Dash_*` (specific to `{bs4Dash}`), `adminlte_*` (for `{shinydashboard}`) or `bs_vars_*` (for classic `{shiny}` apps) functions, which we describe below. 
...
#### Statuses and colors
In Chapter \@ref(sass-customize-bs4Dash), we showed how to change `{bs4Dash}` color, with a few lines of Sass code. Guess what: `{fresh}` makes it even easier! The `bs4dash_status()` and `bs4Dash_color()` functions allow to overwrite all the default statuses and colors:
...
By default, `primary` is blue but could become green with just one line of code. Do you recall section \@ref(sass-customize-bs4Dash), where we customized `{bs4Dash}` colors with Sass? Let's try again with `{fresh}`. We first create the new theme, passing it the new colors and injecting it inside the previously mentioned __freshTheme__ parameter:
...
Compared to the approach described in section \@ref(sass-customize-bs4Dash), there a few advantages:

  - We don't have to specify the `AdminLTE.scss` location, as `{fresh}` hosts it [here](https://github.com/dreamRs/fresh/tree/bf1e00a9d9f2c3388fd88a6a0ca63869b3bf8a81/inst/assets/AdminLTE-3.1.0).
  - We don't have to look for all colors names as they are described along the `bs4dash_color` function definition. We even have a description of the default values (as well as statuses in bold), depicted in Figure \@ref(fig:fresh-colors-defaults) below.
  
```{r fresh-colors-defaults, echo=FALSE, fig.cap='Default {bs4Dash} theme colors.', out.width='50%', fig.align='center'}
...
#### Text color
The card text elements would not properly be styled without setting the `white` color to 
...
#### Color contrast
`bs4dash_yiq()` fine-tunes the [contrast](https://getbootstrap.com/docs/4.0/getting-started/theming/#color-contrast) between a given color and its background. It relies on the Bootstrap 4 `color-yiq` function, whose code may be found [below](https://github.com/dreamRs/fresh/blob/bf1e00a9d9f2c3388fd88a6a0ca63869b3bf8a81/inst/assets/bootstrap-4.5.2/_functions.scss#L70). Interested readers will have a look at the following [article](https://en.wikipedia.org/wiki/Luma_(video)), that explains better where this calculation is derived from.

```{r, echo=FALSE, results='asis'}
css_code <- "@function color-yiq($color, $dark: $yiq-text-dark, 
...
The function has three major steps:
  ...
  - The threshold determines the final color value. If `yiq` is higher than the threshold, the color is black. Conversely, if `yiq` is lower than the threshold, the resulting color is white. 
...
The demonstration may be run with the following code, the result being shown in Figure \@ref(fig:shinydashboardPlus-fresh):

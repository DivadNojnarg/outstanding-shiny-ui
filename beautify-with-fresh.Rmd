...
We'll be using the `navbar_light_*` variables as follows:
  ...
  - `navbar_light_color` set the navbar text and icon elements to white (`#fff`). It is the best
...
```{r}
color_yiq <- "@function color-yiq($color, $threshold: 150, $dark: $yiq-text-dark, $light: $yiq-text-light) {
  $r: red($color);
  $g: green($color);
  $b: blue($color);

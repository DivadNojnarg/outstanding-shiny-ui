...
In this chapter, I'll demonstrate how we can utilize `{htmltools}` [@R-htmltools] to manage web dependencies, that is handling CSS and JS code as well as other assets in your shiny apps. This literally opens the doors to work with almost any web framework and design outstanding user experiences with Shiny. 

## Motivations
In this example, we would like to include a Material Bootstrap [card](https://mdbootstrap.com/docs/standard/components/cards/) in a shiny app. 
...
```{r mdb-card-dirty, echo=FALSE, fig.cap='Attempt to display a Material Bootstrap card without dependencies in a shiny app.', out.width='100%'}
...
The card may seem ugly but at least it is displayed as shown on Figure \@ref(fig:mdb-card-deps). This approach is however not easy to re-usable by other developers since there is no way to recover the specific dependency.

```{r mdp-card-deps, echo=FALSE, fig.cap='Material Bootstrap card within a simple app', out.width='100%'}
...
This raises an important consideration about `htmlDependency()`. Under the hood, Shiny adds many necessary dependencies like Bootstrap, jQuery and you have no control over where your own dependencies will be inserted yet. Section \@ref(htmltools-suppress-deps) explains how to seamlessly solve that issue.
:::
...
You now have limitless possibilities!  Interestingly, this same approach is the basis of [shinyWidgets](https://github.com/dreamRs/shinyWidgets/blob/5b1fe07eaa13ff9157effaa13187d560d4d97644/R/useBs4Dash.R#L123) for the `useBs4Dash()` function and other related tools.
